...
def train(model, score_func, split_edge, train_pos, data, x, optimizer, batch_size, pos_train_weight, data_name, remove_edge_aggre):
    model.train()
    score_func.train()
...
@torch.no_grad()
def test(model, score_func, data, evaluation_edges, x, evaluator_hit, evaluator_mrr, batch_size, use_valedges_as_input):
    model.eval()
    score_func.eval()
...
def main():
    ...
    n2v_emb = torch.load( os.path.join(get_root_dir(), "dataset", args.data_name+'-n2v-embedding.pt'), map_location=torch.device('cpu'))
    x = n2v_emb
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            loss = train(model, score_func, split_edge, pos_train_edge, data, x, optimizer, args.batch_size, train_edge_weight, args.data_name, args.remove_edge_aggre)
           
            
            if epoch % args.eval_steps == 0:
                
                results_rank, score_emb= test(model, score_func, data, evaluation_edges, x, evaluator_hit, evaluator_mrr, args.batch_size, args.use_valedges_as_input)
