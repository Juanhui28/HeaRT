...
def test(model, score_func, data, A, num_nodes, evaluator_hit, evaluator_mrr, batch_size):
    ...
    pos_train_pred, _ = test_edge(score_func, data['train_val'], h, batch_size, alpha, model, A_)
    pos_valid_pred, neg_valid_pred = test_edge(score_func, data['valid_pos'], h, batch_size,  alpha, model, A_, negative_data=data['valid_neg'])
    pos_test_pred, neg_test_pred = test_edge(score_func, data['test_pos'], h, batch_size,  alpha, model, A_, negative_data=data['test_neg'])
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                results_rank, score_emb = test(model, score_func, data, A, node_num,  evaluator_hit, evaluator_mrr, args.batch_size)
