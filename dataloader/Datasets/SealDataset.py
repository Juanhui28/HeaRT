...
class IndexSampler(Sampler):
    ...
    def __iter__(self):
       ...
       return(self.idx[i] for i in range(len(self.idx)))
...
class SEALDataset(InMemoryDataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
                 ...
                 max_nodes_per_hop=None, directed=False, use_valedges_as_input=False):
        self.data = data
    ...
    def process(self):
        ...
        pos_edge, neg_edge = get_pos_neg_edges(self.split, self.split_edge, self.percent)
        
        edge_index, edge_weight = self.data.edge_index, self.data.edge_weight
        if self.use_coalesce:  # compress mutli-edge into edge with weight
            edge_index, edge_weight = coalesce(
                edge_index, edge_weight, 
                self.data.num_nodes, self.data.num_nodes)

        if self.use_valedges_as_input:
            edge_index = self.data.full_edge_index
            edge_weight = self.data.full_edge_weight
        
        A = ssp.csr_matrix(
            (edge_weight.reshape(-1), (edge_index[0], edge_index[1])), 
...
class SEALDynamicDataset(Dataset):
    def __init__(self, root, data, split_edge, num_hops, percent=100, split='train', 
                 use_coalesce=False, node_label='drnl', ratio_per_hop=1.0, 
                 max_nodes_per_hop=None, directed=False, use_valedges_as_input=False, **kwargs):
        self.data = data
        self.split_edge = split_edge
        self.num_hops = num_hops
