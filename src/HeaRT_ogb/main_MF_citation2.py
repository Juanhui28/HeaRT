...
@torch.no_grad()
def test(predictor, x, evaluation_edges, evaluator_mrr, batch_size):
    predictor.eval()
    pos_valid_edge, neg_valid_edge, pos_test_edge,  neg_test_edge = evaluation_edges
...
def main():
    ...
    evaluation_edges = [ pos_valid_edge, neg_valid_edge, pos_test_edge,  neg_test_edge]
    ...
    evaluator = Evaluator(name=args.eval_mrr_data_name)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                results, score_emb = test(predictor, emb.weight, evaluation_edges, evaluator,
                ...
                r = torch.tensor(logger['MRR'].results[run])
    ...
    print(str(best_valid_current) + ' ' + str(best_test) + ' ' + '2' + ' ' + '2')
