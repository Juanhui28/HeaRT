...
def test_edge(score_func, input_data, h, batch_size, alpha, model, A_,  negative_data=None):
    ...
    # edge_index = torch.cat([row.unsqueeze(0), col.unsqueeze(0)],dim=0).to(torch.long)

    # # A_= SparseTensor.from_edge_index(edge_index, values.view(-1), [A_.shape[0], A_.shape[0]]).to(h.device)
    
    # A_ = torch.sparse.FloatTensor(edge_index, values.view(-1), [A_.shape[0], A_.shape[0]] ).to(h.device)
...
def main():
    ...
    if args.data_name =='ogbl-collab' and args.use_2hop:
        A2 = A @ A
        A = A + args.beta*A2
